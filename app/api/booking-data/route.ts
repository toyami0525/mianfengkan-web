...
export async function GET() {
  try {
    ...
    const [{data:staff,error:staffError},{data:reservations,error:reservationError},{data:settings,error:settingsError},{data:closure,error:closureError},{data:calendarOff,error:calendarOffError},{data:serviceAvailability,error:serviceError}] = await Promise.all([
      ...
      db.from('staff_service_availability').select('staff_id,service_name,enabled,staff:staff_id(slug)'),
    ]);
    ...
    const yuki=(staff||[]).find(row=>row.slug==='yukinoji-hakari');
    const yukinojiChibiAccepting=Boolean(yuki&&staffServiceState(yuki,serviceAvailability||[]).available_services.includes('Q版繪圖(公版)'));
    ...
    const availableStaff=(staff||[]).filter((x:any)=>!offIds.has(String(x.id))).map(row=>{
      const state=staffServiceState(row,serviceAvailability||[]);
      return {...row,...state,services:state.available_services};
    });
    const polaroidPausedSlugs=(serviceAvailability||[]).filter(row=>row.service_name==='紀念拍立得'&&!row.enabled).map(row=>{
      const profile=Array.isArray(row.staff)?row.staff[0]:row.staff;
      return profile?.slug;
    }).filter(Boolean);
    return NextResponse.json({staff:availableStaff,blocks,test_mode:testMode,yukinoji_chibi_accepting:yukinojiChibiAccepting,polaroid_paused_slugs:polaroidPausedSlugs,venue_closed:venueClosed,venue_closed_reason:venueClosedReason},{headers:{'Cache-Control':'no-store, max-age=0'}});
  } catch (error) {
