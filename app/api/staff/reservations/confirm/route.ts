...
export async function POST(req:NextRequest){
  try{
    ...
    if(String(reservation.status)!=='pending')return NextResponse.json({error:'這筆服務不是待確認狀態，無法重新開始計時'},{status:409});
    ...
    const startLine=`[實際開始 ${taipeiStamp(now)}] 館員確認後開始計時；原預約時間 ${taipeiStamp(oldStart)}`;
    const nextNote=[reservation.note,startLine].filter(Boolean).join('\n');
    const{data:updated,error:updateError}=await db.from('reservations').update({status:'confirmed',starts_at:newStart.toISOString(),ends_at:newEnd.toISOString(),note:nextNote}).eq('id',reservation.id).eq('status','pending').select('*').maybeSingle();
