-- 眠楓館 v1.5：當日整點指名制＋館員指名結束後 5 分鐘緩衝
...
begin
  ...
  if extract(minute from v_taipei_start) <> 0 or extract(second from v_taipei_start) <> 0 then
    raise exception '指名採 60 分鐘整點時段制，只能選擇 21:00、22:00 或 23:00';
  end if;
  ...
  -- 新預約開始時間必須在既有預約結束至少 5 分鐘後；
  -- 若新預約排在既有預約之前，新預約結束後也必須保留 5 分鐘。
  if exists (
    ...
    raise exception '此時段已有其他預約，館員需在每筆指名結束後保留 5 分鐘';
  end if;
