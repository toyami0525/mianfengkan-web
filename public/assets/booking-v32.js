...
function deliveryStaffCanBeNominated(staffId){const delivery=deliveryStaff.find(s=>s.id===staffId);if(delivery?.slug==='riku'||delivery?.name==='羽鶴璃久')return true;return staff.some(s=>s.id===staffId)}
function availableDeliveryStaff(){return deliveryStaff.filter(s=>deliveryStaffCanBeNominated(s.id)&&!deliveryStaffIsBusy(s.id)&&!deliveryStaffIsUnavailable(s.id))}
