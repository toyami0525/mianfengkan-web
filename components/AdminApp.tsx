...
export default function AdminApp(){
 ...
 const[calendarMonth,setCalendarMonth]=useState(()=>monthKey(new Date())),[calendarRows,setCalendarRows]=useState<Row[]>([]),[calendarStaff,setCalendarStaff]=useState<Row[]>([]),[calendarStaffId,setCalendarStaffId]=useState(''),[calendarLoading,setCalendarLoading]=useState(false);
 ...
 async function loadWorkCalendar(month=calendarMonth){
  ...
  try{
   ...
   const staffRows=Array.isArray(result.staff)?result.staff:[],scheduleRows=Array.isArray(result.rows)?result.rows:[];
   setCalendarStaff(staffRows);setCalendarRows(scheduleRows);
...
function WorkCalendar({month,staffId,rows,busy,onChange}:{month:string,staffId:string,rows:Row[],busy:string,onChange:(staffId:string,date:string,status:'working'|'off'|null)=>void}){
 const cells=calendarCells(month);const byDate=new Map(rows.filter(x=>x.staff_id===staffId).map(x=>[x.work_date,x.status]));
 return <div className="work-calendar"><div className="work-calendar-weekdays">{['一','二','三','四','五','六','日'].map(x=><b key={x}>{x}</b>)}</div><div className="work-calendar-grid">{cells.map((date,i)=>{if(!date)return <span className="calendar-day empty" key={`empty-${i}`}/>;const status=byDate.get(date)||'unset';const next=status==='unset'?'working':status==='working'?'off':null;return <button type="button" key={date} disabled={!staffId||busy===`calendar:${date}`} className={`calendar-day ${status}`} onClick={()=>onChange(staffId,date,next)}><strong>{Number(date.slice(-2))}</strong><small>{status==='working'?'上班':status==='off'?'休假':'未設定'}</small></button>})}</div></div>
}
