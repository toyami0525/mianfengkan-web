...
const labels:Record<string,string>={guest_name:'客人',contact:'聯絡方式',staff_name:'指名館員',service_name:'服務',price:'服務金額',starts_at:'指名時間',status:'狀態',note:'備註',items:'點餐內容',total:'總金額',created_at:'送出時間',name:'姓名',role:'職位',active:'前台顯示',accepting_reservations:'接受指名',staff_id:'館員 ID',ends_at:'結束時間',reason:'原因',title:'標題',content:'內容',published:'公開',key:'設定項目',value:'設定內容',updated_at:'更新時間',pickup_code:'取件碼',delivery_preference_staff_name:'希望送餐',delivery_staff_name:'實際送餐'};
...
export default function AdminApp(){
 ...
 useEffect(()=>{
  ...
  const tick=()=>{
   ...
   const rows=(data.reservations||[]).filter(r=>{
    if(['completed','cancelled','rejected','已完成','已取消','已拒絕'].includes(String(r.status)))return false;
 ...
 function canExtendReservation(reservation:Row){
  ...
  if(!['pending','confirmed','已確認'].includes(String(reservation.status)))return false;
