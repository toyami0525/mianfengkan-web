...
function liquorClass(name:any){const text=String(name||'');if(text==='純米酒・宵紅葉')return 'liquor-red';if(text==='純米吟釀・月白')return 'liquor-blue';if(text==='本釀造・青嵐')return 'liquor-green';if(text==='生酒・夢紫')return 'liquor-purple';if(text==='梅酒・金梅')return 'liquor-yellow';return ''}
function formatItems(value:any){let items=value;try{if(typeof items==='string')items=JSON.parse(items)}catch{}if(!Array.isArray(items))return String(value??'—');return <div className="order-items">{items.map((item:any,i:number)=>{const color=liquorClass(item.name);return <div key={i} className={color?`liquor-admin-item ${color}`:''}><strong>{item.name||'未命名品項'}</strong><span> × {item.qty??1}</span>{item.price!=null&&<small>（{Number(item.price).toLocaleString('zh-TW')} Gil）</small>}</div>})}</div>}
