...
const ownerTabs=[['dashboard','總覽'],['payroll','今日薪資'],['staff','館員管理'],['schedule','排班／請假'],['operations','營運管理'],['polaroids','拍立得管理'],['announcements','公告管理'],['rules','規章管理'],['settings','網站設定']];
const staffTabs=[['dashboard','我的總覽'],['operations','營運管理'],['polaroids','我的拍立得']];
...
const statusText:Record<string,string>={pending:'待確認',accepted:'已確認・待開始',confirmed:'服務中',completed:'已完成',cancelled:'已取消',rejected:'已拒絕'};
...
export default function AdminAppV259d(){
 ...
 useEffect(()=>{
  ...
  const dateKey=localDateInput(new Date());
 ...
 async function acknowledgeReservation(id:string){
  const key=`reservations:${id}`;setBusy(key);setMsg('確認指名中…');
  try{
   const{data:{session}}=await db.auth.getSession();if(!session)throw new Error('登入已失效，請重新登入');
   const response=await fetch('/api/staff/reservations/acknowledge',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify({reservation_id:id})});
   const result=await response.json();if(!response.ok)throw new Error(result.error||'確認指名失敗');
   ...
   setMsg('指名已確認。等客人與館員都準備好後，再按「開始」才會正式計時。');notifyPublicBookingChange('booking_changed');await load();
  }catch(error){const text=error instanceof Error?error.message:'確認指名失敗';console.error(error);setMsg(`確認失敗：${text}`);alert(`確認失敗：${text}`)}finally{setBusy('')}
 }
 ...
 function savePayrollRoster(next:string[]){
  ...
  localStorage.setItem(`mf-payroll-roster:${localDateInput(new Date())}`,JSON.stringify(safe));
 ...
 function resetPayrollRoster(){
  ...
  const allStaff=(data.staff||[]).filter(r=>r.active!==false);
  const {ownerId}=payrollOwnerInfo();
  const dateKey=localDateInput(new Date()),shiftStart=toTaipeiIso(dateKey,'21:00'),shiftEnd=toTaipeiIso(dateKey,'24:00');
  const offIds=new Set<string>();
  if(shiftStart&&shiftEnd){
   const a=new Date(shiftStart).getTime(),b=new Date(shiftEnd).getTime();
   for(const row of data.staff_unavailability||[]){
 ...
 {tab==='operations'&&<div className="operations-grid">
  ...
  <div className="operations-column operations-reservations"><Panel title={account.role==='staff'?'我的指名管理':'指名管理'}><RecordToolbar range={reservationRange} status={reservationStatus} search={reservationSearch} onRange={v=>{setReservationRange(v);setReservationPage(1)}} onStatus={v=>{setReservationStatus(v);setReservationPage(1)}} onSearch={v=>{setReservationSearch(v);setReservationPage(1)}} statuses={['pending','accepted','confirmed','completed','cancelled','all']}/><RecordSummary total={filteredReservations.length} range={reservationRange} status={reservationStatus}/><Table rows={visibleReservations} keys={['guest_name','contact','staff_name','service_name','starts_at','ends_at','price','status','note']} actions={r=><>{r.status==='pending'&&account.role!=='frontdesk'&&<button disabled={busy===`reservations:${r.id}`} onClick={()=>void acknowledgeReservation(r.id)}>確認</button>}{r.status==='accepted'&&account.role!=='frontdesk'&&<button className="start-button" disabled={busy===`reservations:${r.id}`} onClick={()=>void status('reservations',r.id,'confirmed')}>開始</button>}{canExtendReservation(r)&&<button className="extend-button" disabled={busy===`extend:${r.id}`} onClick={()=>openExtend(r)}>續時</button>}<button disabled={busy===`reservations:${r.id}`} onClick={()=>status('reservations',r.id,'completed')}>完成</button><button disabled={busy===`reservations:${r.id}`} onClick={()=>status('reservations',r.id,'cancelled')}>取消</button></>}/><Pagination page={reservationPage} pages={reservationPages} onChange={setReservationPage}/></Panel></div>
 </div>}
 {tab==='payroll'&&account.role==='owner'&&<><section className="payroll-stats"><article><span>今日餐點收入</span><b>{payrollFoodRevenue.toLocaleString('zh-TW')} <small>Gil</small></b><em>已排除取消訂單與拍立得加購</em></article><article><span>今日上班人數</span><b>{payrollHeadcount} <small>人</small></b><em>包含館主本人</em></article><article><span>每人餐點分紅</span><b>{payrollFoodShare.toLocaleString('zh-TW')} <small>Gil</small></b><em>餐點收入 ÷ 上班人數</em></article><article className="payroll-highlight"><span>每人今日薪資</span><b>{payrollPerPerson.toLocaleString('zh-TW')} <small>Gil</small></b><em>基本 150,000 ＋ 餐點分紅</em></article></section><Panel title="今日上班人員" action={<button className="ghost" onClick={resetPayrollRoster}>依今日請假重新判定</button>}><p className="hint">館主固定計入。系統第一次會依今日 21:00～24:00 的請假／不可指名時段預設排除館員；若只是短暫離開或實際仍有上班，可在這裡手動勾回。名單會記在這台裝置今天的後台。</p><div className="payroll-roster">{payrollPeople.map(person=><label key={person.id} className={person.isOwner?'owner':''}><input type="checkbox" checked={person.isOwner||payrollStaffIds.includes(person.id)} disabled={person.isOwner} onChange={()=>togglePayrollStaff(person.id)}/><span><strong>{person.name}</strong><small>{person.isOwner?'館主・固定計入':'館員'}</small></span></label>)}</div></Panel><Panel title="今日薪資明細"><div className="payroll-formula"><span>基本薪資</span><strong>{BASE_DAILY_PAY.toLocaleString('zh-TW')} Gil</strong><b>＋</b><span>每人餐點分紅</span><strong>{payrollFoodShare.toLocaleString('zh-TW')} Gil</strong><b>＝</b><span>每人應得</span><strong>{payrollPerPerson.toLocaleString('zh-TW')} Gil</strong></div><div className="table-wrap"><table className="payroll-table"><thead><tr><th>人員</th><th>基本薪資</th><th>餐點分紅</th><th>今日應得</th></tr></thead><tbody>{selectedPayrollPeople.map(person=><tr key={person.id}><td><strong>{person.name}</strong>{person.isOwner&&<small>（館主）</small>}</td><td>{BASE_DAILY_PAY.toLocaleString('zh-TW')}</td><td>{payrollFoodShare.toLocaleString('zh-TW')}</td><td><strong>{payrollPerPerson.toLocaleString('zh-TW')} Gil</strong></td></tr>)}</tbody></table></div>{payrollRemainder>0&&<p className="payroll-remainder">餐點收入平均後剩餘 <strong>{payrollRemainder.toLocaleString('zh-TW')} Gil</strong> 無法整除，這筆尾數未加入任何人的薪資。</p>}<div className="payroll-payout"><div><span>其他館員人數</span><strong>{payrollEmployeeCount} 人</strong></div><div className="total"><span>今天實際需要發給其他館員</span><strong>{payrollCashOut.toLocaleString('zh-TW')} Gil</strong></div><small>館主自己的 {payrollPerPerson.toLocaleString('zh-TW')} Gil 顯示在明細中，但不計入「要支付出去」的金額。</small></div></Panel></>}
...
function Table({rows,keys,actions}:{rows:Row[],keys:string[],actions?:(r:Row)=>React.ReactNode}){if(!rows.length)return <div className="empty">目前沒有符合條件的資料</div>;return <div className="table-wrap"><table><thead><tr>{actions&&<th className="actions-sticky-head">操作</th>}{keys.map(k=><th key={k}>{labels[k]||k}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={r.id||i}>{actions&&<td className="actions-sticky-cell"><div className="actions">{actions(r)}</div></td>}{keys.map(k=><td key={k}>{formatCell(k,r[k])}</td>)}</tr>)}</tbody></table></div>}
